...
Config.prototype._loggingManuallySet = function _loggingManuallySet(inputConfig) {
  ...
  return (inputEnabled || process.env.NEW_RELIC_LOG_ENABLED) !== undefined
}
...
Config.prototype._enforceServerless = function _enforceServerless(inputConfig) {
  if (this.serverless_mode.enabled) {
    ...
    // If DT and serverless_mode are set, enforce that account id is set.
    if (this.distributed_tracing.enabled && !this.account_id) {
      logger.warn(
...
Config.prototype._canonicalize = function _canonicalize() {
  ...
  this.transaction_tracer.disable_hrtime = this.transaction_tracer.disable_hrtime
    && this.feature_flag.disable_hrtime
}
