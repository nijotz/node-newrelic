...
class TransactionTraceAggregator extends TraceAggregator {
  ...
  */
  resetTimingTracker() {
    logger.trace('reseting timing tracker')
    this.requestTimes = Object.create(null)
    this.noTraceSubmitted = 0
  }
  ...
  */
  add(transaction) {
    if (!transaction) {
      logger.trace('no transaction provided')
      return
    }
  ...
  */
  isBetter(name, duration, apdexT) {
    ...
    */
    ...
    if (!isOverThreshold) {
      logger.trace('not over threshold')
      return false
    }
    ...
    */
    ...
    if (!slowerThanExisting) {
      logger.trace('not slower than existing')
      return false
    }
    ...
    */
    ...
    if (!slowerThanCaptured) {
      logger.trace('not slower than captured')
      return false
    }
