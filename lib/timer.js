...
function Timer(config) {
  this._disableHrTime = config.feature_flag.disable_hrtime
  this.state = PENDING
  this.touched = false
  this._duration = null
  this._start = null
 ...
 *
 * Uses process.hrtime by default if available, Date.now() otherwise,
 * or if configured via feature flag.
 */
Timer.prototype.begin = function begin() {
  if (this.state > PENDING) return

  this.start = Date.now()
  this._start = this._disableHrTime ? this.start : process.hrtime()
  this.state = RUNNING
}
...
Timer.prototype.end = function end() {
  ...
  this._duration = this._getDuration(this._start)
  this.touched = true
  this.state = STOPPED
}

Timer.prototype._getDuration = function _getDuration(start) {
...
Timer.prototype.setDurationInMillis = function setDurationInMillis(duration, start) {
  ...
  // this is fine.
  this._start = this._disableHrTime
    ? Date.now()
    : [Math.floor(start / 1e3), start % 1e3 * 1e6]
  if (start != null) {
    this.start = start
  }
}
...
Timer.prototype.getDurationInMillis = function getDurationInMillis() {
  ...
  if (this._duration) {
    return this._disableHrTime ? this._duration : hrToMillis(this._duration)
  }

  const duration = this._getDuration(this._start)

  return this._disableHrTime ? duration : hrToMillis(duration)
}
...
Timer.prototype.startedRelativeTo = function startedRelativeTo(other) {
  if (!this._disableHrTime && this._start && other._start) {
    var s = this._start[0] - other._start[0]
    var ns = this._start[1] - other._start[1]
